...
libcurl does not support the use of extensions and will always setup a
...
1. WRITEFUNCTION model: Using a write callback with CURLOPT_WRITEFUNCTION(3)
   much like other downloads for when the traffic is download oriented.

2. CONNECT_ONLY model: Using curl_ws_recv(3) and curl_ws_send(3) functions.
...
the user knows WebSocket and can parse and extract the data all by themself.
